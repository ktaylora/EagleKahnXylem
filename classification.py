...
def shuffle(df: pd.DataFrame, balance_classes: bool = False,
    ...
    categories: Any = np.unique(df[response])
    if balance_classes:
        ...
        series = df.sample(n=len(df))
    # wrap a pandas call to optionally split our dataframe
    # into testing / training combinations if a frac= argument
    # was applied
    if frac is not 1.0:
        train_set = series.copy().sample(frac=frac)
        test_set = series.copy().drop(train_set.index)  # type: ignore
        return {'training': train_set, 'testing': test_set}
    else:
        return {'training': series}


def mean_center(df: pd.DataFrame) -> Dict:
    """ mean-variance center continuous variables in a
     dataframe as a pre-cursor to least-squares regression.
     This is important if you want to analyze effects
     sizes from the fitted regression parameters."""
    r = df.copy()
    return {
        'mean': r.mean(),
        'sd': r.std(),
        'scaled': r.subtract(r.mean()).divide(r.std())}
...
def encode_design_matrix(df: pd.DataFrame, variables: list = [], 
    ...
    """ accepts an input pandas dataframe and optionally normalizes columns and
     creates dummy variables for categoricals as a pre-cursor to fitting a
     model in scikit"""
    if normalize:
        normalized: Any = mean_center(df[variables])
    else:
        normalized: Any = df[variables]
    if len(dummy_variables) > 0:
        ...
        x = pd.get_dummies(x[variables + dummy_variables]).\
            to_numpy().\
    ...
    if normalize:
        result['scale'] = {
            k: normalized[k] for k in ('mean','sd')}
...
def fit_random_forests(df, variables: list = [], dummy_variables: list = [],
                       response: str = "Survived", normalize: bool = False,
                       **kwargs):
    """ wrapper for RandomForestClassifier that will omit normalization
     of an input pandas dataframe and encode dummy variables prior to fitting
     a classification model. This currently doesn't change any of the default
     options for RF under the hood, but could. """
...
def predict(m: Any, x: pd.DataFrame, variables: List[str] = [], type: str = ""):
...
def cross_validation(results: list, k_folds: list, variables: list,
                     dummy_variables: list = [], apply_scale: bool = False,
                     response: str ="Survived"):
    """ assess accuracy and error for a series of k classification model
     replicates using k-folds cross-validation """

    for i in range(len(results)):
        testing_df: pd.DataFrame = k_folds[i]['testing'].dropna()
        ...
        if apply_scale:
            ...
            if len(dummy_variables) > 0:
                design_matrix = encode_design_matrix(
                    scale(testing_df[variables], scale_params).join(
                        testing_df[dummy_variables]), 
                    variables=variables, dummy_variables=dummy_variables,
                    normalize=False)
            else:
                design_matrix = encode_design_matrix(
    ...
    pass  # end super-long cross-validation for-loop


def std_effect_sizes(results: list):
    """ accepts a bootstrapped list of models from the user and uses the fitted
     coefficients from the models to estimate mean and standard error of it's
     input parameters """
    n: int = len(results)
...
def ga_bitflipping(predictions: Any):
    """ """
    pass
